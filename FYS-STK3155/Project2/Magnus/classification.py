...
def gridSearch(X_train, X_test, Y_train, Y_test, epochs, n_neuron, n_layers, OneHot=True):
    for i, eta in enumerate(eta_vals):
        for j, lmbd in enumerate(lmbd_vals):
            dnn = NeuralNetwork(np.copy(X_train), np.copy(Y_train), n_layers, n_neuron, RELU, RELU_deriv, epochs = epochs, eta = eta, lmbd=lmbd, Type="Classification")
    ...
    #plotting
    plot_data(eta_vals, lmbd_vals, Train_accuracy, 'RELU')
    plot_data(eta_vals, lmbd_vals, Test_accuracy, 'RELU')
...
gridSearch(X_train, X_test, Y_train, Y_test, 50, n_neuron, n_layers, OneHot=True)