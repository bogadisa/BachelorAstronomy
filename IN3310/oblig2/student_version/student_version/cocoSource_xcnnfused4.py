...
class ImageCaptionModel(nn.Module):
    def __init__(self, config: dict):
        ...
        self.input_layer = nn.Sequential(nn.Dropout(p=0.25), nn.Conv1d(self.number_of_cnn_features, self.nn_map_size, 1), nn.BatchNorm1d(self.nn_map_size), nn.LeakyReLU())
    ...
    def forward(self, cnn_features, x_tokens, is_train: bool, current_hidden_state=None) -> tuple:
        ...
        # print(torch.transpose(cnn_features, 1, 2).shape)
        # exit()

        maxpool = nn.MaxPool1d(10)

        processed_cnn_features = torch.transpose(self.input_layer(torch.transpose(cnn_features, 1, 2)), 1, 2)

        # print(processed_cnn_features.shape)
        # processed_cnn_features = maxpool(processed_cnn_features.clone()).squeeze()
        # print(processed_cnn_features.shape)
        # exit()
        ...
        if current_hidden_state is None:
            ...
            initial_hidden_state = torch.zeros(self.num_rnn_layers, cnn_features.size(dim=0), self.hidden_state_sizes).to(device="cuda:0")
        else:
...
class RNNOneLayerSimplified(nn.Module):
    ...
    def forward(self, tokens, processed_cnn_features, initial_hidden_state, output_layer: nn.Linear,
        ...
        for i in range(sequence_length):
            ...
            input_for_the_first_layer = torch.cat((processed_cnn_features, current_time_step_embeddings), dim=1) #current_time_step_embeddings
            ...
            current_hidden_state = self.cells[0](input_for_the_first_layer, current_hidden_state.squeeze()).unsqueeze(dim=0)
...
class RNN(nn.Module):
    def __init__(self, input_size, hidden_state_size, last_layer_state_size, num_rnn_layers, cell_type='GRU'):
        """
        ...
        self.attention = nn.Sequential(nn.Dropout(p=0.25), nn.Linear(hidden_state_size, 50), nn.LeakyReLU(), nn.Linear(50, 10), nn.Softmax())
        ...
        input_size_list = [input_size] + [hidden_state_size] * (num_rnn_layers - 2) + [last_layer_state_size]
        ...
        if cell_type == "GRU":
            self.cells = nn.ModuleList([GRUCell(hidden_state_size, input_size_list[i]) for i in range(num_rnn_layers)])
        elif cell_type == "RNN":
            self.cells = nn.ModuleList([RNNsimpleCell(hidden_state_size, input_size_list[i]) for i in range(num_rnn_layers)])
        if cell_type == "LSTM":
            self.cells = nn.ModuleList([LSTMCell(hidden_state_size, input_size_list[0], self.attention)])
            self.cells = nn.ModuleList([LSTMCell(hidden_state_size, input_size_list[i]) for i in range(1, num_rnn_layers)])
            # self.cells.append(LSTMCell(hidden_state_size, input_size_list[-1], self.attention))

        else:
            print(f"{cell_type=} is not implemented")
            print("Exiting...")
            quit()
    ...
    def forward(self, tokens, processed_cnn_features, initial_hidden_state, output_layer: nn.Linear,
        ...
        # current_hidden_state = torch.cat((initial_hidden_state, initial_hidden_state), dim=2)
        current_hidden_state = initial_hidden_state
        
        maxpool = nn.MaxPool1d(10)
        pooled_cnn_features = maxpool(torch.transpose(processed_cnn_features.clone(), 1, 2)).squeeze()
        # self.input_layer(torch.transpose(cnn_features, 1, 2))
        ...
        for i in range(sequence_length):
            ...
            input_for_the_first_layer = torch.cat((pooled_cnn_features, input_tokens), dim=1)
            output_from_the_first_layer = self.cells[0](input_for_the_first_layer, current_hidden_state[0, :, :].clone()).unsqueeze(dim=0)
            # current_hidden_state[0] = 
            for j in range(1, self.num_rnn_layers):
                current_hidden_state[j] = self.cells[j](current_hidden_state[j-1, :, :self.hidden_state_size], current_hidden_state[j, :, :].clone()).unsqueeze(dim=0)
            # current_hidden_state[-1]

            logits_i = output_layer(current_hidden_state[-1, :, :self.hidden_state_size].clone())
            logits_sequence.append(logits_i)
            # Find the next predicted output element
            predictions = torch.argmax(logits_i, dim=1)
            ...
            if i < sequence_length - 1:
                if is_train:
                    ...
                    input_tokens = embedding_layer(predictions)  # embedding_layer(predictions)
...
class GRUCell(nn.Module):
    def __init__(self, hidden_state_size: int, input_size: int):
        ...
        self.weight_u = nn.Parameter(
            torch.randn(hidden_state_size + input_size, hidden_state_size) / np.sqrt(input_size + hidden_state_size))
    ...
    def forward(self, x, hidden_state):
        ...
        ht_ = torch.tanh(torch.mm(x3, self.weight) + self.bias)

        new_hidden_state = zt*hidden_state + (1 - zt)*ht_
...
class LSTMCell(nn.Module):
    def __init__(self, hidden_state_size: int, input_size: int, attention=None):
        """
        :param hidden_state_size: Size (number of units/features) in the hidden state of GRU
        ...
        self.weight = nn.Parameter(
            torch.randn(hidden_state_size + input_size, hidden_state_size) / np.sqrt(input_size + hidden_state_size))
        self.bias = nn.Parameter(torch.zeros(1, hidden_state_size) + 2)
    ...
    def forward(self, x, hidden_state):
        ...
        print(x.shape)
        x2 = torch.cat((x, hidden_state[:, :self.hidden_state_size]), dim=1)
        print(x2.shape)
        # if self.attention is None:
        #     x2 = torch.cat((x, hidden_state[:, :self.hidden_state_size]), dim=1)
        # else:
        #     weights_a = self.attention(x)
        #     x2 = torch.cat((x, weights_a), dim=1)
        ...
        gt = torch.tanh(torch.mm(x2, self.weight) + self.bias)

        ct = ft*hidden_state[:, self.hidden_state_size:] + it*gt
        if self.attention is None:
            x3 = ot*torch.tanh(ct)
        else:
            x3 = self.attention(ot*torch.tanh(ct))
            print()
            print(x.shape)
            print(x3.shape)
            print("bye")
        ...
        new_hidden_state = torch.cat((x3, ct), dim=1)
        return new_hidden_state
