...
class ImageCaptionModel(nn.Module):
    def __init__(self, config: dict):
        ...
        # self.input_layer = nn.Sequential(nn.Dropout(p=0.25), nn.Linear(self.number_of_cnn_features, self.nn_map_size), nn.LeakyReLU())
        ...
        if self.simplified_rnn:
            ...
            self.rnn = RNN(input_size=self.embedding_size + self.nn_map_size,
                           hidden_state_size=self.hidden_state_sizes,
                           last_layer_state_size=self.hidden_state_sizes,
    ...
    def forward(self, cnn_features, x_tokens, is_train: bool, current_hidden_state=None) -> tuple:
        ...
        if current_hidden_state is None:
            ...
            initial_hidden_state = torch.zeros(self.num_rnn_layers, cnn_features.size(dim=0), 2*self.hidden_state_sizes).to(device="cuda:0")
        else:
...
class RNN(nn.Module):
    def __init__(self, input_size, hidden_state_size, last_layer_state_size, num_rnn_layers, cell_type='GRU'):
        ...
        if cell_type == "LSTM":
            self.cells = nn.ModuleList([LSTMCell(hidden_state_size, input_size_list[i]) for i in range(num_rnn_layers)])

            self.attention_head = nn.Sequential(nn.Dropout(p=0.25), 
    ...
    def forward(self, tokens, processed_cnn_features, initial_hidden_state, output_layer: nn.Linear,
        ...
        pooled_cnn_features = maxpool(processed_cnn_features).squeeze()
        ...
        for i in range(sequence_length):
            ...
            current_hidden_state[0] = self.cells[0](input_for_the_first_layer, current_hidden_state[0, :, :].clone()).unsqueeze(dim=0)
            # current_hidden_state[0] =
            for j in range(1, self.num_rnn_layers-1):
            ...
            j = self.num_rnn_layers - 2
            attention_weights = self.attention_head(current_hidden_state[j])
            # print()
            # print(attention_weights.shape)
            context = torch.transpose(processed_cnn_features, 0, 1)*attention_weights
            # print(context.shape)
            context_vector = torch.transpose(torch.sum(context, 2), 0, 1)
            # print(context_vector.shape)

            # input_for_final_lstm_layer = torch.cat((current_hidden_state[j], context_vector), dim=1)
            input_for_final_lstm_layer = context_vector
            # print(input_for_final_lstm_layer.shape)
            # print(current_hidden_state[-1].shape)

            current_hidden_state[-1] = self.cells[-1](input_for_final_lstm_layer, current_hidden_state[-1, :, :].clone()).unsqueeze(dim=0)
...
class LSTMCell(nn.Module):
    ...
    def forward(self, x, hidden_state):
        ...
        new_hidden_state = torch.cat((ot*torch.tanh(ct), ct), dim=1)
        return new_hidden_state
