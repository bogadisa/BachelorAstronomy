...
for i, [x, y] in enumerate(ABCD):
    wavelength_spectrum[i] = idata[y-1,x-1,:]
    # model = g(wavelength_func(xi), a[i], b[i], c[i], d[i])
    # model = g(wavelength_func(xi), a[i], b[i], c[i], d[i])
    
    # boundaries = ([a[i]-0.2*abs(a[i]), b[i]-0.2*abs(b[i]), c[i]-0.2*abs(c[i]), d[i]-0.2*abs(d[i])], [a[i]+0.2*abs(a[i]), b[i]+0.2*abs(b[i]), c[i]+0.2*abs(c[i]), d[i]+0.2*abs(d[i])])
    #print(boundaries)
    # print (wavelength_func(xi))
    # print (wavelength_spectrum)
    # popt, pcov = curve_fit(g, wavelength_func(xi), model, bounds=boundaries) #
    
    popt, pcov = curve_fit(g, wavelength_func, wavelength_spectrum[i], [a[i], b[i], c[i], d[i]])
    # break
    
    
    # print(popt, [a[i], b[i], c[i], d[i]])
    #ax.flatten()[i].plot(wavelength_spectrum_avg, ls="--", lw=1, marker="x", label="Average spectra")
    ax.flatten()[i].plot(wavelength_func, wavelength_spectrum[i], ls="--", lw=1, marker="x", label="Spectra at point")
