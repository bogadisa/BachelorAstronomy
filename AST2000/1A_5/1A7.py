...
#funksjonskallene samt alle definerte variabler ligger nederst i filen
class Planet:
...
class Rocket(Planet):
    ...
    def simulate(self, dt, n):
        self.vel=self.particles.vel
        self.pos=self.particles.pos
        pos=self.pos
        vel=self.vel
        ...
        for i in range(n):
            ...
            pos=np.where(pos>=L, L-(pos-L), pos)
    ...
    def fuel_loss(self, periode):
        walls=self.walls
        n_boxes=self.n_boxes
        dt, n, L, m, N, mass=self.dt, self.n, self.L, self.m, self.N, self.mass
        dT=dt*n
        self.dT=dT
        loss=n_boxes*m*walls[2]*(periode)/dT
        print(f"Raketten krever {loss:.0f}kg drivstoff for å nå {self.v_esc():.0f}m/s")
        self.loss, self.periode=loss, periode
    
    def launch(self, t_steps):
        walls=self.walls
        dT, L, m, N, mass=self.dT, self.L, self.m, self.N, self.mass
        loss, periode=self.loss, self.periode
        loss_per_second=loss/periode
        mass_rocket=mass+loss
        dt=periode/t_steps
        v=0
        while mass_rocket>mass and v<self.v_esc():
            for i in range(t_steps):
                mass_rocket-=loss_per_second
                F=walls[3]/dT
                dv=F/mass_rocket
                v+=dv*dt
        else:
            if mass_rocket>mass:
                print(f"Raketten nådde en hastighet {v} før den gikk tom for drivstoff")
...
#enda flere variabler
periode=20*60 #20minuter i sekunder
t_steps=1000  #antall tidsteg brukt på å skytte opp raketten

#finner hvor mye drivstoff vi bruker
model1.fuel_loss(periode)
model1.launch(t_steps)